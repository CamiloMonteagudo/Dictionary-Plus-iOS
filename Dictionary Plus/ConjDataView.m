...
- (void) UpdateInActualViewMode
  ...
  int Cols = (self.bounds.size.width + SEP) / (ConjCore.WMax + 10 + SEP);
  if( Cols <= 0 ) Cols = 1;
  
  [self MakeCellsForCols:Cols Heigth:ConjCore.HMax+10 Count:(int)CnjCells.count];
...
- (void)layoutSubviews
  ...
  int Cols = (W + SEP) / (ConjCore.WMax + 10 + SEP);
  if( Cols >= 0 ) Cols = 1;
  
  int H = ConjCore.HMax + 10;
