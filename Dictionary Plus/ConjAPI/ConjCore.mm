...
+(bool) ConjVerb: (NSString *)sVerb
  ...
  if( !_IsVerb ) return false;                                              // No es un verbo
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------
// Obtiene el infinitivo de verbo 'sVerb' si no es un verbo retorna cadena vacia
+(NSString *) VerbInfinitive: (NSString *)sVerb Lang:(int) lng
  {
  if( sVerb.length == 0 ) return FALSE;
  ...
  if( !_IsVerb ) return @"";                                               // No es un verbo

  return [NSString stringWithCString:Verb.c_str() encoding:NSISOLatin1StringEncoding ];
  }
...
+(NSArray *) SortByConjList:(NSArray *) lstData
  ...
  NSString* lastCnj = @"";
  NSComparisonResult ret = NSOrderedSame;              // Resultado de la comparación
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------
// Obtiene una lista de todas las conjugaciones obtenidas
+(NSArray<NSString*> *) GetConjsList
  {
  ...
  for( int i=0; i<_CData.Count; ++i )
    {
    cjData& cnj = _CData.Conjs[i];
    if( cnj.Conj=="-" || cnj.Conj.Length()==0 ) continue;

    NSString* sConj = [NSString stringWithCString:cnj.Conj.c_str() encoding:NSISOLatin1StringEncoding ];

    ret = [lastCnj compare:sConj options:FindOpt];    // Compara la palabra buscada con la actual
    if( ret == NSOrderedSame ) continue;

    if( ret == NSOrderedDescending && i>2)                 // Si es inferior a la última
      {
      for( int j=3; j<List.count; ++j)
        {
        ret = [sConj compare:List[j] options:FindOpt];
        if( ret == NSOrderedSame ) break;
        if( ret == NSOrderedAscending )
          {
          [List insertObject:sConj atIndex:j];
          break;
          }
        }
      }
      ...
      {
      [List addObject:sConj];
      if( i>2 ) lastCnj = sConj;
      }
    }
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------
static CGFloat FontSize = [UIFont systemFontSize]+2;

static UIFont* fontMeanBold      = [UIFont boldSystemFontOfSize: FontSize];
static UIFont* fontConjBold      = [UIFont boldSystemFontOfSize: FontSize];
static UIFont* fontConjSmall     = [UIFont systemFontOfSize: 0.75 *FontSize];
static UIFont* fontConjBoldSmall = [UIFont boldSystemFontOfSize: 0.75 *FontSize];
static UIFont* fontConjBoldMid   = [UIFont boldSystemFontOfSize: 0.9*FontSize];

static UIColor* ColMean     = [UIColor blackColor];                                                             // Color para el cuerpo del significado
static UIColor* colConj     = [UIColor blackColor];
static UIColor* colConjMode = [UIColor colorWithRed:0.06 green:0.43 blue:0.06 alpha:1.00];
static UIColor* colConjPers = [UIColor colorWithRed:0.06 green:0.06 blue:0.43 alpha:1.00];
static UIColor* colConjGray = [UIColor darkGrayColor];
static UIColor* ColErrInfo  = [UIColor colorWithRed:0.6 green:0.2 blue:0.2 alpha:1.00];                        // Color para mostrar infomación de errores
...
static NSDictionary* attrModeSmall   = @{ NSFontAttributeName:fontConjSmall   , NSForegroundColorAttributeName:colConjMode };
...
static NSDictionary* attrPersSmall   = @{ NSFontAttributeName:fontConjSmall , NSForegroundColorAttributeName:colConjPers };

static NSDictionary* attrConjGray    = @{ NSFontAttributeName:fontConjSmall, NSForegroundColorAttributeName:colConjGray };
...
+(NSAttributedString*) TextFromConj:(int) idx TwoLines:(BOOL) two
  ...
  AddNSString(Str, Strs.PersNum2[i], attrPersSmall);
...
+(NSAttributedString*) GroupModeTimeFrom:(int) idx
  ...
  for( int i=0; i<6; ++i )
    ...
    AddNSString(Str, Strs.PersNum1[i], attrPersSmall);
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------
static CGFloat FontSz = [UIFont systemFontSize];

static UIFont* fontCj          = [UIFont systemFontOfSize:    FontSz];
static UIFont* fontCjBold      = [UIFont boldSystemFontOfSize: FontSz];
static UIFont* fontCjSmall     = [UIFont systemFontOfSize: 0.9 *FontSz];
static UIFont* fontCjBoldSmall = [UIFont boldSystemFontOfSize: 0.9 *FontSz];

static NSDictionary* attrCj          = @{ NSFontAttributeName:fontCj         , NSForegroundColorAttributeName:colConj     };
static NSDictionary* attrCjBold      = @{ NSFontAttributeName:fontCjBold     , NSForegroundColorAttributeName:colConjMode };
static NSDictionary* attrCjSmall     = @{ NSFontAttributeName:fontCjSmall    , NSForegroundColorAttributeName:colConj     };
static NSDictionary* attrCjBoldSmall = @{ NSFontAttributeName:fontCjBoldSmall, NSForegroundColorAttributeName:colConj     };

//--------------------------------------------------------------------------------------------------------------------------------------------------------
+(NSAttributedString*) GetConjList:(NSArray<NSString*>*) List ForVerb:(NSString*) Wrd
  {
  NSString* lbConjs = NSLocalizedString(@"lbConjations", nil);
  
  NSMutableAttributedString* Str = [[NSMutableAttributedString alloc] init];

  AddNSString(Str, LGFlag(_CData.cLang), attrCj );
  AddNSString(Str, List[0], attrCjBold );
  AddNSString(Str, lbConjs, attrConjGray );
  AddNSString(Str, @"\r\n", attrCjBold );
  
  int n = (int)List.count;
  for( int i=1; i<n; ++i )
    {
    NSString* word = List[i];
   
    if( [Wrd isEqualToString:word ]) AddNSString(Str, word, attrCjBoldSmall);
    else                             AddNSString(Str, word, attrCjSmall    );
    
    if( i<n-1) AddNSString(Str, @", ", attrCjSmall);
    }
...
+(void) AddGenAndNumStr:(NSMutableAttributedString*) Str Root:(LPCSTR)sRoot Type:(int)Tipo Gener:(int)Genero Number:(int)Numero
  ...
  AddNSString(Str, Strs.Gens[Genero], attrPersSmall);
  AddNSString(Str, @" "             , attrConjBold);
  
  AddNSString(Str, Strs.Nums3[Numero], attrPersSmall);
...
+(void) AddVerboRootStr:(NSMutableAttributedString*) Str Root:(LPCSTR)sRoot Mood:(int)Modo Time:(int)Tiempo Person:(int)Persona Number:(int)Numero With:(float) wSize
  ...
  if( i>2 )
    ...
    float w2 = [PerLarge sizeWithAttributes:attrPersSmall].width;
    
    if( w1 + w2 < wSize )                                                     // Si hay espacio
      AddNSString(sAttr, PerLarge , attrPersSmall );                          // Pone la descrición completa
    else
      AddNSString(sAttr, Strs.PersNum1[idx], attrPersSmall );                 // Pone abreviaturas
    }
