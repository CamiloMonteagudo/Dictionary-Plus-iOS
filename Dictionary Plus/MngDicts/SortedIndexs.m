...
//===================================================================================================================================================
@interface SortedIndexs()
  {
  NSInteger nWrds;                                        // Número de palabras de la consulta
  }
@end
...
-(void) AddEntry:(NSNumber*) idxEntry WithPos:(INT_LIST*) WrdsPos
  {
  int rank = [self RankForEntry2:idxEntry AndWrdsPos:WrdsPos ];
...
-(int) RankForEntry:(NSNumber*) idxEntry AndWrdsPos:(INT_LIST*) WrdsPos
  ...
  int del   = (int)nWrds - found;                                 // Cantidad de palabras que hay quitar del query
...
///------------------------------------------------------------------------------------------------------------------------------------------------------------------
/// Calcula el ranking de la entrada 'idxEntry'
-(int) RankForEntry2:(NSNumber*) idxEntry AndWrdsPos:(INT_LIST*) WrdsPos
  {
  int words = (int)Dict.Items[idxEntry.intValue].nWrds;           // Numero de palabra de la llave
  ...
  [WrdsPos sortUsingComparator:^NSComparisonResult(NSNumber *obj1, NSNumber *obj2)
    {
  ...
  int desp   = 0;                                                 // Cantidad de desplazamientos que hay que realizar
  int offset = WrdsPos[0].intValue;                               // Separación de la frase respecto al inicio
  ...
  for( int i=1; i<found; i++ )                                  // Recorre todas las demas palabras
    ...
    if(dt<0)
      NSLog(@"Hay una posición negativa");
    }
