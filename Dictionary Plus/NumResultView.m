...
- (void) initData
  ...
  _wPanel = self.frame.size.width;
  
  CGPoint pos = self.frame.origin;
  self.frame  = CGRectMake(pos.x, pos.y, _wPanel, LineHeight );
...
//  float  wText = wPanel-2*SEP_BRD - 2*SEP_TXT;                   // Calcula ancho del control del texto
  CGRect    rc = CGRectMake(SEP_BRD+SEP_TXT, SEP_TXT, 300, LineHeight);
...
-(UILabel*) CreateLabelWithText:(NSString*) txt
  {
  CGRect  rc = CGRectMake(0, 0, _wPanel, LineHeight);                                  // Cualquier frame, en el layout se recalcuala
...
- (void) SetGroupType
  ...
  [self SetNumberReadingInLang:lastLang];
  }
...
- (void)layoutSubviews
  ...
  if( hView != _hPanel || wView != _wPanel )
    {
    _hPanel = hView;
    _wPanel = wView;
    
    //CGPoint pos = self.frame.origin;
    //self.frame = CGRectMake(pos.x, pos.y, _wPanel, _hPanel);
    ...
    SelType.frame = CGRectMake((_wPanel-szSel.width)/2, y, szSel.width, szSel.height );
    ...
    if( self.Delegate )
      [self.Delegate OnNumResultChagedSize:self];
    }
  }
...
- (void)drawRect:(CGRect)rect
  {
  CGRect rc = CGRectMake(SEP_BRD, 0, _wPanel-(2*SEP_BRD), _hPanel );
