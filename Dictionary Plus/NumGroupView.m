...
//--------------------------------------------------------------------------------------------------------------------------------------------------------
// Inicializa la vista donde se va introducir el número a leer
- (void) initData
  {
  ...
  TextCtl.returnKeyType    = UIReturnKeyDone;
  TextCtl.enablesReturnKeyAutomatically = TRUE;
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
  ...
  if( num==1 && text.length==0 )                             // Esta borrando
    ...
    delSpc = (c==' ');
    if( c<23 )
      NSLog(@"Caracter de control = %d", c);
    }
  else if( num==0 && [text isEqualToString:@"\n"] )         // Oprimio return
    {
    HideKeyboard();
    return false;
    }
...
- (void)textViewDidBeginEditing:(UITextView *)textView
  {
  nowEdit = textView;                                 // Pone la vista en edición en una varible global, para ocultar el teclado
  }
